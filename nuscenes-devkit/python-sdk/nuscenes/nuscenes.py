...
class NuScenes:
    ...
    def get_sample_data(self, sample_data_token: str,
        ...
        for box in boxes:
            if use_flat_vehicle_coordinates:
                ...
                box.translate(-np.array(pose_record['translation'], dtype=np.float64))
                ...
                box.translate(-np.array(cs_record['translation'], dtype=np.float64))
    ...
    def render_sample(self, sample_token: str,
        ...
        self.explorer.render_sample(sample_token, box_vis_level, nsweeps=nsweeps, out_path=out_path,
                                    show_lidarseg=show_lidarseg, show_lidarseg_legend=show_lidarseg_legend,
                                    filter_lidarseg_labels=filter_lidarseg_labels,
    ...
    def render_scene(self, scene_token: str, freq: float = 10, imsize: Tuple[float, float] = (640, 360),
                     out_path: str = None, verbose: bool = True) -> None:
        self.explorer.render_scene(scene_token, freq, imsize, out_path, verbose)
        ...
        if len(lidar_data) > 0:
            for (_, sd_token), ax in zip(lidar_data.items(), axes.flatten()[num_radar_plots:]):
                self.render_sample_data(sd_token, box_vis_level=box_vis_level, ax=ax, nsweeps=nsweeps,
                                        show_lidarseg=show_lidarseg, show_lidarseg_legend=show_lidarseg_legend, filter_lidarseg_labels=filter_lidarseg_labels,
