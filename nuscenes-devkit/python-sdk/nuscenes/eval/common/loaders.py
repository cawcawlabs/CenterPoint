...
def load_gt_from_predictions(nusc: NuScenes, prediction_sample_tokens: Set[str], verbose: bool = False) -> EvalBoxes:
    """
    Loads ground truth boxes from DB.
    :param nusc: A NuScenes instance.
    :param prediction_sample_tokens: The predicted sample_tokens from which we load GT boxes.
    :param verbose: Whether to print messages to stdout.
    :return: The GT boxes.
    """
    all_annotations = EvalBoxes()
    ...
    # Load annotations and filter predictions and annotations.
    for sample_token in tqdm.tqdm(prediction_sample_tokens, leave=verbose):
        sample = nusc.get('sample', sample_token)
        sample_annotation_tokens = sample['anns']
        sample_boxes = []
        for sample_annotation_token in sample_annotation_tokens:
